...
public:
	Maths() {
		...
		constexpr float aspectRatio = 800.0f / 600.0f; // example aspect ratio
		constexpr float  fov         = glm::radians(45.0f); // example field of view
		constexpr float  nearPlane   = 0.1f;                // example near plane
		constexpr float   farPlane    = 1000.0f;             // example far plane
		...
		constexpr float angle   = glm::radians(45.0f); // example angle
		...
		constexpr float rotationSpeed = glm::radians(30.0f); // example rotation speed (30 degrees per second)
		...
		cbObj.rotationMat = cameraProjMat * cameraViewMat * objectWorldMat; // Model-View-Projection matrix
