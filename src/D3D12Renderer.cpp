...
D3D12Renderer::D3D12Renderer(int width, int height)
    : _width(width)
    , _height(height)
    , _running(true)
{
...
bool D3D12Renderer::InitD3D(HWND hwnd)
{
  HRESULT hr;
  ...
  swapChainDesc.Windowed             = true; // Assuming windowed mode

  ComPtr<IDXGISwapChain> tempSwapChain;
  ThrowIfFailed(dxgiFactory->CreateSwapChain(commandQueue.Get(), &swapChainDesc, &tempSwapChain));

  ThrowIfFailed(tempSwapChain.As(&swapChain)); // Use As method for IDXGISwapChain3
  ...
  if (!CompileShader(L"../shader/VertexShader.hlsl", 
      ShaderType::Vertex, &vertexShader, &errorBuff))
  {
...
void D3D12Renderer::UpdateVertexBuffer(const std::vector<Vertex>& vertices)
{
  if (!vertexBuffer)
  {
...
bool D3D12Renderer::InitializeVertexBuffer(const std::vector<Vertex>& vertices)
{
  try
  {
    if (vertices.empty())
...
void D3D12Renderer::UpdateConstantBuffer(const glm::mat4& rotationMat, const glm::mat4& projectionMat,
...
void D3D12Renderer::InitializeComputeBuffer(const std::vector<Vertex>& vertices)
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE D3D12Renderer::GetRTVHandle()
{
  CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(GetRTVDescriptorHeap()->GetCPUDescriptorHandleForHeapStart(), 
                                          GetFrameIndex(),
                                          GetRTVDescriptorSize());
  return rtvHandle;
}

HfovxyFocal D3D12Renderer::calculateHfovxyFocal(float fovy, float _height, float _width)
{
  HfovxyFocal result;
...
void D3D12Renderer::UpdatePipeline()
{
  ...
  ThrowIfFailed(commandList->Reset(commandAllocator[frameIndex].Get(), pipelineStateObject.Get()));
  ...
  imguiAdapter->commandList(commandList.Get());
...
void D3D12Renderer::ExecuteComputeShader()
{
  HRESULT hr;
...
void D3D12Renderer::drawUI()
{
  // Implement your UI drawing logic here
}
